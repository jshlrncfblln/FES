...
class Action
{
	...
	{
		extract($_POST);
		$type = array("", "users", "faculty_list", "student_list", "program_head_list", "dean_list", "registrar_list");
		$type2 = array("", "admin", "faculty", "student", "program_head", "dean", "registrar");
		$qry = $this->db->query("SELECT *,concat(firstname,' ',lastname) as name FROM {$type[$login]} where school_id = '" . $email . "' and password = '" . md5($password) . "'  ");
		if ($qry->num_rows > 0) {
